...
<div class="space-y-8 max-w-4xl mx-auto">
  <header>
    ...
    <p class="mt-2 text-gray-500 dark:text-gray-400">Configure your API keys and application preferences.</p>
  </header>

  <form (submit)="saveSettings($event)">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md">
      <div class="p-6 space-y-6">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-white">LLM API Keys</h2>
        @for(provider of providers; track provider.id) {
          <div>
            <label [for]="provider.id" class="block text-sm font-medium text-gray-600 dark:text-gray-300">{{ provider.name }}</label>
            @if(provider.id === 'google') {
               <input type="text" [id]="provider.id" [name]="provider.id" disabled [value]="apiKeys()[provider.id]"
                   class="mt-1 block w-full bg-gray-200 dark:bg-gray-900 border-gray-300 dark:border-gray-700 rounded-md shadow-sm sm:text-sm text-gray-500 dark:text-gray-400 cursor-not-allowed h-10 px-3">
                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">The Google Gemini API key is securely managed via the 'API_KEY' environment variable and cannot be changed here.</p>
            } @else {
      ...
      <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 text-right rounded-b-lg">
        <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 dark:hover:bg-indigo-500 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-200">
          Save Settings
        </button>
      </div>
    </div>
